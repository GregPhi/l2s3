...
import coding
import struct
from tp04_francaix_philippot import write_binary_string_in_file
from tp04_francaix_philippot import read_file
...
def write_occurrences(occurrences, filename):
    ...
    for key in occurrences:
        stream.write(bytes([ord(key)]))
        stream.write(struct.pack('i', occurrences[key]))
    stream.close()
...
def read_occurrences(filename):
    ...
    byte = stream.read(1)
    while byte != b'':
        occurrences[byte] = struct.unpack('i', stream.read(size))[0]
        byte = stream.read(1)
    stream.close()
    return occurrences
...
def huffman_encode(filename, out_filename):
    ...
    tree = create_huffman_tree(occurrences)
    coding = huffman_coding(tree)
    ...
    byte = stream.read(1)
    while byte != b'':
        binary += coding.code(byte)
        byte = stream.read(1)
    write_binary_string_in_file(binary, out_filename)
    stream.close()
...
def huffman_decode(filename, out_filename):
    ...
    occurences = read_occurrences(filename + '.code')
    # Lecture du fichiers à décoder
    bits = read_file(filename)
    # Création de l'arbre de huffman
    tree = create_huffman_tree(occurences)
    decode = prefix_tree_decoding(bits, tree)
    # ecriture
